...
IDE_Morph.prototype.openIn = function (world) {
    ...
    /*
     * this.cloudMsg = getURL('http://snap.berkeley.edu/cloudmsg.txt'); motd =
     * getURL('http://snap.berkeley.edu/motd.txt'); if (motd) {
...
IDE_Morph.prototype.createPalette = function (forSearching) {
	...
	// search toolbar (floating cancel button):
	    /*
	     * commented out for now this.palette.toolBar = new PushButtonMorph(
	     * this, function () { myself.refreshPalette();
	     * myself.palette.adjustScrollBars(); }, new
	     * SymbolMorph("magnifierOutline", 16) ); this.palette.toolBar.alpha =
	     * 0.2; this.palette.toolBar.padding = 1; // this.palette.toolBar.hint =
	     * 'Cancel'; this.palette.toolBar.labelShadowColor = new Color(140, 140,
	     * 140); this.palette.toolBar.drawNew();
	...
	if (droppedMorph instanceof DialogBoxMorph) {
	     ...
	     */
	    // console.log('suppr
	    // lutin',droppedMorph.name,droppedMorph.version);
	     ...
	     */
	    // console.log('suppr
	    // sptriteicon',droppedMorph.labelString,droppedMorph.version);
	    sendEvenement(type='ENV',
	    ...
	    /**
	     * Modification JML (duff, 29 déc. 2017) note: fait aussi un stopall
	     */
...
IDE_Morph.prototype.stopAllScripts = function () {
    /**
     * Modification JML (duff, 30 déc. 2017) envoi de l'évènement + un snapshot
     */
...
IDE_Morph.prototype.removeSprite = function (sprite) {
    var idx, myself = this;
    /**
     * Modification JML (duff, 29 déc. 2017) provenance de la suppression dans
     * sprite.JMLfrom: 'Drop', 'Drop Icon', 'Menu', 'Remove Clone', et dans ce
     * cas JMLblock=block d'origine
     */
...
IDE_Morph.prototype.settingsMenu = function () {
    ...
    menu.addLine();
    /*
     * addPreference( 'JavaScript', function () { Process.prototype.enableJS =
     * !Process.prototype.enableJS; myself.currentSprite.blocksCache.operators =
     * null; myself.currentSprite.paletteCache.operators = null;
     * myself.refreshPalette(); }, Process.prototype.enableJS, 'uncheck to
     * disable support for\nnative JavaScript functions', 'check to
    ...
    /*
     * addPreference( 'Input sliders', 'toggleInputSliders',
     * MorphicPreferences.useSliderForInput, 'uncheck to disable\ninput sliders
     * for\nentry fields', 'check to enable\ninput sliders for\nentry fields' );
     * if (MorphicPreferences.useSliderForInput) { addPreference( 'Execute on
     * slider change', 'toggleSliderExecute',
     * ArgMorph.prototype.executeOnSliderEdit, 'uncheck to suppress\nrunning
     * scripts\nwhen moving the slider', 'check to run\nthe edited script\nwhen
     * moving the slider' ); } addPreference( 'Turbo mode',
     * 'toggleFastTracking', this.stage.isFastTracked, 'uncheck to run
     * scripts\nat normal speed', 'check to prioritize\nscript execution' );
     * addPreference( 'Visible stepping', 'toggleSingleStepping',
     * Process.prototype.enableSingleStepping, 'uncheck to turn off\nvisible
     * stepping', 'check to turn on\n visible stepping (slow)', false );
     * addPreference( 'Ternary Boolean slots', function () {
     * BooleanSlotMorph.prototype.isTernary =
     * !BooleanSlotMorph.prototype.isTernary; },
     * BooleanSlotMorph.prototype.isTernary, 'uncheck to limit\nBoolean slots to
     * true / false', 'check to allow\nempty Boolean slots', true );
     * addPreference( 'Camera support', 'toggleCameraSupport',
     * CamSnapshotDialogMorph.prototype.enableCamera, 'uncheck to
     * disable\ncamera support', 'check to enable\ncamera support', true );
     * menu.addLine(); // everything visible below is persistent addPreference(
     * 'Blurred shadows', 'toggleBlurredShadows', useBlurredShadows, 'uncheck to
     * use solid drop\nshadows and highlights', 'check to use blurred
     * drop\nshadows and highlights', true ); addPreference( 'Zebra coloring',
     * 'toggleZebraColoring', BlockMorph.prototype.zebraContrast, 'uncheck to
     * disable alternating\ncolors for nested block', 'check to enable
     * alternating\ncolors for nested blocks', true ); addPreference( 'Dynamic
     * input labels', 'toggleDynamicInputLabels',
     * SyntaxElementMorph.prototype.dynamicInputLabels, 'uncheck to disable
     * dynamic\nlabels for variadic inputs', 'check to enable dynamic\nlabels
     * for variadic inputs', true ); addPreference( 'Prefer empty slot drops',
     * 'togglePreferEmptySlotDrops',
     * ScriptsMorph.prototype.isPreferringEmptySlots, 'uncheck to allow
     * dropped\nreporters to kick out others', 'settings menu prefer empty slots
     * hint', true ); addPreference( 'Long form input dialog',
     * 'toggleLongFormInputDialog',
     * InputSlotDialogMorph.prototype.isLaunchingExpanded, 'uncheck to use the
     * input\ndialog in short form', 'check to always show slot\ntypes in the
     * input dialog' ); addPreference( 'Plain prototype labels',
     * 'togglePlainPrototypeLabels',
     * BlockLabelPlaceHolderMorph.prototype.plainLabel, 'uncheck to always show
     * (+) symbols\nin block prototype labels', 'check to hide (+) symbols\nin
     * block prototype labels' ); addPreference( 'Virtual keyboard',
     * 'toggleVirtualKeyboard', MorphicPreferences.useVirtualKeyboard, 'uncheck
     * to disable\nvirtual keyboard support\nfor mobile devices', 'check to
     * enable\nvirtual keyboard support\nfor mobile devices', true );
     * addPreference( 'Clicking sound', function () {
     * BlockMorph.prototype.toggleSnapSound(); if
     * (BlockMorph.prototype.snapSound) { myself.saveSetting('click', true); }
     * else { myself.removeSetting('click'); } },
     * BlockMorph.prototype.snapSound, 'uncheck to turn\nblock clicking\nsound
     * off', 'check to turn\nblock clicking\nsound on' ); addPreference(
     * 'Animations', function () {myself.isAnimating = !myself.isAnimating; },
     * myself.isAnimating, 'uncheck to disable\nIDE animations', 'check to
     * enable\nIDE animations', true ); addPreference( 'Cache Inputs', function () {
     * BlockMorph.prototype.isCachingInputs =
     * !BlockMorph.prototype.isCachingInputs; },
     * BlockMorph.prototype.isCachingInputs, 'uncheck to stop caching\ninputs
     * (for debugging the evaluator)', 'check to cache inputs\nboosts
     * recursion', true ); addPreference( 'Rasterize SVGs', function () {
     * MorphicPreferences.rasterizeSVGs = !MorphicPreferences.rasterizeSVGs; },
     * MorphicPreferences.rasterizeSVGs, 'uncheck for smooth\nscaling of vector
     * costumes', 'check to rasterize\nSVGs on import', true ); addPreference(
     * 'Flat design', function () { if (MorphicPreferences.isFlat) { return
     * myself.defaultDesign(); } myself.flatDesign(); },
     * MorphicPreferences.isFlat, 'uncheck for default\nGUI design', 'check for
     * alternative\nGUI design', false ); addPreference( 'Nested auto-wrapping',
     * function () { ScriptsMorph.prototype.enableNestedAutoWrapping =
     * !ScriptsMorph.prototype.enableNestedAutoWrapping; if
     * (ScriptsMorph.prototype.enableNestedAutoWrapping) {
     * myself.removeSetting('autowrapping'); } else {
     * myself.saveSetting('autowrapping', false); } },
     * ScriptsMorph.prototype.enableNestedAutoWrapping, 'uncheck to confine
     * auto-wrapping\nto top-level block stacks', 'check to enable
     * auto-wrapping\ninside nested block stacks', true ); addPreference(
     * 'Project URLs', function () { myself.projectsInURLs =
     * !myself.projectsInURLs; if (myself.projectsInURLs) {
     * myself.saveSetting('longurls', true); } else {
     * myself.removeSetting('longurls'); } }, myself.projectsInURLs, 'uncheck to
     * disable\nproject data in URLs', 'check to enable\nproject data in URLs',
     * true ); addPreference( 'Sprite Nesting', function () {
     * SpriteMorph.prototype.enableNesting =
     * !SpriteMorph.prototype.enableNesting; },
     * SpriteMorph.prototype.enableNesting, 'uncheck to disable\nsprite
     * composition', 'check to enable\nsprite composition', true );
     * addPreference( 'First-Class Sprites', function () {
     ...
     * myself.refreshPalette(); }, SpriteMorph.prototype.enableFirstClass,
     * 'uncheck to disable support\nfor first-class sprites', 'check to enable
     * support\n for first-class sprite', true );
     */    
    /**
     * Fin Modification JML
     */
    ...
    /*
     * modif jml addPreference( 'Table support', function () {
     * List.prototype.enableTables = !List.prototype.enableTables; if
     * (List.prototype.enableTables) { myself.removeSetting('tables'); } else {
     * myself.saveSetting('tables', false); } }, List.prototype.enableTables,
     * 'uncheck to disable\nmulti-column list views', 'check for
     * multi-column\nlist view support', true ); if
     * (List.prototype.enableTables) { addPreference( 'Table lines', function () {
     * TableMorph.prototype.highContrast = !TableMorph.prototype.highContrast;
     * if (TableMorph.prototype.highContrast) { myself.saveSetting('tableLines',
     * true); } else { myself.removeSetting('tableLines'); } },
     * TableMorph.prototype.highContrast, 'uncheck for less
     * contrast\nmulti-column list views', 'check for higher contrast\ntable
     * views', true ); } addPreference( 'Live coding support', function () {
     * Process.prototype.enableLiveCoding = !Process.prototype.enableLiveCoding; },
     * Process.prototype.enableLiveCoding, 'EXPERIMENTAL! uncheck to disable
     * live\ncustom control structures', 'EXPERIMENTAL! check to enable\n live
     * custom control structures', true ); menu.addLine(); // everything below
     * this line is stored in the project addPreference( 'Thread safe scripts',
     * function () {stage.isThreadSafe = !stage.isThreadSafe; },
     * this.stage.isThreadSafe, 'uncheck to allow\nscript reentrance', 'check to
     * disallow\nscript reentrance' ); addPreference( 'Prefer smooth
     * animations', 'toggleVariableFrameRate', StageMorph.prototype.frameRate,
     * 'uncheck for greater speed\nat variable frame rates', 'check for smooth,
     * predictable\nanimations across computers', true ); addPreference( 'Flat
     * line ends', function () { SpriteMorph.prototype.useFlatLineEnds =
     * !SpriteMorph.prototype.useFlatLineEnds; },
     * SpriteMorph.prototype.useFlatLineEnds, 'uncheck for round ends of lines',
     * 'check for flat ends of lines' ); addPreference( 'Codification support',
     * function () { StageMorph.prototype.enableCodeMapping =
     * !StageMorph.prototype.enableCodeMapping;
     * myself.currentSprite.blocksCache.variables = null;
     * myself.currentSprite.paletteCache.variables = null;
     * myself.refreshPalette(); }, StageMorph.prototype.enableCodeMapping,
     * 'uncheck to disable\nblock to text mapping features', 'check for
     * block\nto text mapping features', false ); addPreference( 'Inheritance
     * support', function () { StageMorph.prototype.enableInheritance =
     * !StageMorph.prototype.enableInheritance;
     * myself.currentSprite.blocksCache.variables = null;
     * myself.currentSprite.paletteCache.variables = null;
     * myself.refreshPalette(); }, StageMorph.prototype.enableInheritance,
     * 'uncheck to disable\nsprite inheritance features', 'check for
     * sprite\ninheritance features', false ); addPreference( 'Persist linked
     * sublist IDs', function () { StageMorph.prototype.enableSublistIDs =
     * !StageMorph.prototype.enableSublistIDs; },
     * StageMorph.prototype.enableSublistIDs, 'uncheck to disable\nsaving linked
     * sublist identities', 'check to enable\nsaving linked sublist identities',
     * true );
     */
    menu.popup(world, pos);
};
...
IDE_Morph.prototype.projectMenu = function () {
    ...
    menu.addItem(
		...
		// window.open('cd','nom_de_ma_popup','menubar=no,
		// scrollbars=no, top=100, left=100, width=300,
		// height=200');
	    } ,
    ...
    menu.addLine();
    /*
     * suppression des menus inutiles menu.addItem( 'Import tools', function () {
     * myself.getURL( myself.resourceURL('tools.xml'), function (txt) {
     * myself.droppedText(txt, 'tools'); } ); }, 'load the official library
     * of\npowerful blocks' ); menu.addItem( 'Libraries...', function() {
     * myself.getURL( myself.resourceURL('libraries', 'LIBRARIES'), function
     * (txt) { var libraries = myself.parseResourceFile(txt); new
     * LibraryImportDialogMorph(myself, libraries).popUp(); } ); }, 'Select
     * categories of additional blocks to add to this project.' );
     * 
     * menu.addItem( localize(graphicsName) + '...', function () {
     * myself.importMedia(graphicsName); }, 'Select a costume from the media
     * library' ); menu.addItem( localize('Sounds') + '...', function () {
     * myself.importMedia('Sounds'); }, 'Select a sound from the media library' );
     */
    menu.popup(world, pos);
};
...
IDE_Morph.prototype.exportProjectSummary = function (useDropShadows) {
    ...
    /*
     * if (SnapCloud.username) { add(localize('by ') + SnapCloud.username); }
     */
...
IDE_Morph.prototype.initIds = function(sendScripts=true) {
	...
	if (sendScripts) {
		...
		block.inputs().forEach(function(input) {
			...
			// console.log("input
			// "+input.constructor.name,input.inputs()[0]);
 ...
 * ("+this.JMLid+","+this.JMLdroppedId+")-"+this.selector); //affichage des
 * inputs if (this.inputs()) console.log(d+">INPUTS");
 * this.inputs().forEach(function(i){ i.printStruct(decal+1);
 * //console.log(d+">>"+i); }); if (this.nextBlock && this.nextBlock()) {
 * console.log(d+'/NEXTBLOCK'); this.nextBlock().printStruct(decal); }
 */
/**
 * Fin Modification JML
 */
...
IDE_Morph.prototype.saveFileAs = function (
    ...
    /**
     * Allow for downloading a file to a disk. This relies the FileSaver.js
     * library which exports saveAs() Two utility methods saveImageAs and
     * saveXMLAs should be used first.
     */
...
IDE_Morph.prototype.userSetBlocksScale = function () {
    ...
    action = function (num) {
	/*
	 * var c; blck.setScale(num); blck.drawNew(); blck.setSpec(blck.blockSpec);
	 * c = blck.inputs()[0]; c.setScale(num); c.nestedBlock(scrpt);
	 */
...
IDE_Morph.prototype.createCloudAccount = function () {
    ...
    world = this.world();
    /*
     * // force-logout, commented out for now: delete localStorage['-snap-user'];
     * SnapCloud.clear();
     */
    new DialogBoxMorph(
	    null,
...
IDE_Morph.prototype.cloudError = function () {
     ...
     * request.open('GET', url, false); request.send(); if (request.status ===
     * 200) { return request.responseText; } return null; } catch (err) { return
     * null; } }
     */
    ...
    SnapCloud.reconnect(
		...
		SnapCloud.callService(
			...
			function (response) {
			     ...
			     * myself.ide.download(response); return; }
			     */
 ...
 * thumbnail of the sprite I'm respresenting, and a self-updating label of the
 * sprite's name (in case it is changed elsewhere)
 */
...
SpriteIconMorph.prototype.userMenu = function () {
    ...
    if (StageMorph.prototype.enableInheritance) {
	/*
	 * version that hides refactoring capability unless shift-clicked if
	 * (this.world().currentKey === 16) { // shift-clicked menu.addItem(
	 * "parent...", 'chooseExemplar', null, new Color(100, 0, 0) ); }
	 */
...
/*
 * I am a selectable element in the SpriteEditor's "Costumes" tab, keeping a
 * self-updating thumbnail of the costume I'm respresenting, and a self-updating
 * label of the costume's name (in case it is changed elsewhere)
 */
...
CostumeIconMorph.prototype.removeCostume = function () {
    ...
    off = CamSnapshotDialogMorph.prototype.enableCamera ? 3 : 2,
	    ide = this.parentThatIsA(IDE_Morph);
    wardrobe.removeCostumeAt(idx - off); // ignore paintbrush and camera
    // buttons
...
/*
 * I am a selectable element in the SpriteEditor's "Costumes" tab, keeping a
 * thumbnail of the sprite's or stage's default "Turtle" costume.
 */
...
/*
 * SoundIconMorph.prototype.step = SpriteIconMorph.prototype.step;
 */
...
//  CamSnapshotDialogMorph ////////////////////////////////////////////////////
    ...
    /*
     * I am a dialog morph that lets users take a snapshot using their webcam and
     * use it as a costume for their sprites or a background for the Stage.
     * 
     * NOTE: Currently disabled because of issues with experimental technology in
     * Safari.
     */
...
//  CamSnapshotDialogMorph settings
    ...
    CamSnapshotDialogMorph.prototype.enableCamera = true; // off while
    // experimental
...
//  CamSnapshotDialogMorph instance creation
	...
	this.videoView = new Morph(); // a morph where we'll copy the video
	// contents
