import {data,xsend,url} from './xsend.js'
...
var findSelected=function() {
    ...
    f=[]
    sel.each(function(p,j){p.data.forEach(function(e){f.push(e.session_key)})})
    return f
}
...
var resetClasseDate=function() {
    ...
    var s=d3.select("#classeDate").select("#sessionsChart"),
    sel=s.selectAll(".session .selected")
    sel.each(function(p,j){d3.select(this).classed("selected",false)})
...
var selectAllClasseDate=function() {
    ...
    sel.each(function(p,j){d3.select(this).classed("selected",true)})
    var f=findSelected()
    tableChart.dimension().filterAll()
...
// console.log("par classe",sessionsClasse)
        ...
        var sessionsLimites=d3.nest().key(d=>`${d.classe_nom}(${d.classe_id})`).sortKeys(d3.ascending).key(d=>d3.timeHour(d.debut)).sortKeys((a,b)=>new Date(a)- new Date(b)).rollup(function(l){ return {"values":l,"min":d3.min(l,d=>d.debut),"max":d3.max(l,d=>d.fin),
...
// console.log('classes,heures,jours',classes,heures,jours)
        ...
        sessionsHeures.forEach(function(d){
            d.values.forEach(function(e){
                dataHeures.push({"classe":d.key,"heure":new Date(e.key),"data":e.values})
            })
        })
        ...
        .append("title")
        .text(function(d){ return "classe de "+d.classe.split("(")[0]+":\n"
...
// creation de la table
        ...
        // .showGroups(false)
        .columns(['classe',
            {label:"user",format:function(d){return d.user_nom}},
            {label:"date",format:function(d){
                return d3.timeFormat("%x")(d.debut)
            }},
            {label:'début',format:d=>d3.timeFormat("%X")(d.debut)},
            {label:'fin',format:d=>d3.timeFormat("%X")(d.fin)},
            {label:'session',format:d=>d.session_key},
            {label:'evts',format:d=>d.nb_evts},
            ])
            .sortBy(d=>d.user)
...
// choix de la visualisation
    ...
    .on("click",function() {
        ...
        xsend(url,csrf_token,{"type":z,"data":liste},"POST").then(response => console.log('Sucscess:', response,JSON.stringify(response)))
    })

    d3.select("#selectAllClasseAnchor").on("click",function() {selectAllClasseDate()})
    d3.select("#resetClasseAnchor").on("click",function() {resetClasseDate()})
    d3.select("#selectAllSessionsAnchor").on("click",function() {selectAllSessions()})
    d3.select("#resetSessionsAnchor").on("click",function() {resetSessions()})
}
export {lance}